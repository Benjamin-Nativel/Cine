...
<div x-data="{ modelOpen: false }" class="mx-6 ">
    <button @click="modelOpen =!modelOpen" class="flex items-center justify-center px-3 py-2 my-2 space-x-2 text-sm tracking-wide text-white capitalize transition-colors duration-200 transform bg-indigo-500 rounded-md dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:bg-indigo-700 hover:bg-indigo-600 focus:outline-none focus:bg-indigo-500 focus:ring focus:ring-indigo-300 focus:ring-opacity-50">
...
<!-- component -->
<div class="m-5 text-2xl font-bold text-center text-gray-800 heading">{{$film->titre}}</div>
<style>
    body {
...
@if ($errors->any())
    ...
    @endforeach
@endif
<form action="{{ Route('edition',$film['id']) }}" method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <div class="flex flex-col w-10/12 max-w-2xl mx-auto text-gray-800 editor -lg">
        ...
        <option value="{{ $real->id }}"
            ...
            @if($real->id === $film->id_real) selected
                                             @endif  value="{{$real->id}}"> 
    ...
    <select name="salle" class="p-2 mb-4 bg-gray-100 border border-gray-300 outline-none"
        ...
        @foreach ($salle as $salle)
            <option value="{{ $salle->id }}"
                @if($salle->id === $film->id_salle) selected
                  @endif  value="{{$salle->id}}"> 
                                         {{ $salle->num_salle }} 
            </option>
        @endforeach
        ...
        <input class="p-2 mb-4 bg-gray-100 border border-gray-300 outline-none title" spellcheck="false"
            placeholder="Title" type="text" name="titre" value="{{$film->titre}}" >
            <input class="p-2 mb-4 bg-gray-100 border border-gray-300 outline-none title" spellcheck="false"
            placeholder="" type="file" name="image" required >
            <input class="p-2 mb-4 bg-gray-100 border border-gray-300 outline-none title" spellcheck="false"
            placeholder="Durée" type="text" name="tps" value="{{$film->duree}}" >
        <textarea class="p-3 bg-gray-100 border border-gray-300 outline-none description sec h-60" spellcheck="false"
            placeholder="Ecriver votre extrait ici" name="resume" >{{$film->resume}}</textarea>
